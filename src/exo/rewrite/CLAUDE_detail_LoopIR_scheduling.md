* `DoUnsafeRemoveIf(stmt_c, recursive=True)`: fixed 2026-09-25 in `exo`.
  ...
  No generated-code change for the current Sm90 gemm; the wgmma-zero prototypes in [plan_wgmma_zero_alternatives.md](../../../plan_wgmma_zero_alternatives.md) relied on the old bug (workaround in [plan_wgmma_zero.md](../../../plan_wgmma_zero.md)) to keep `if iter_k == 0` and would need explicit guard removal.
